...
SELECT staff.first_name, staff.last_name, COUNT(payment.payment_id) AS 'Total rung up in Agust 2005 per staff'
FROM staff
JOIN payment
USING(staff_id)
...
SELECT film.title As Film, count(film_actor.actor_id) AS 'Number of Actors played'
FROM film_actor
JOIN film USING (film_id)
GROUP BY film_id;

-- 6d. How many copies of the film Hunchback Impossible exist in the inventory system?
SELECT COUNT(inventory.film_id) AS 'Hunchback Impossible Copies in Inventory'
FROM inventory
WHERE film_id IN (
	SELECT film_id
    FROM film
...
-- or without using koin. i found that way prettier
...
SELECT f.title AS 'Most frequently rented movies', COUNT(r.inventory_id) AS 'Number of Rented Times'
FROM film f
...
SELECT store.store_id, CONCAT(first_name, ' ', staff.last_name) AS 'Store Manager', SUM(payment.amount) AS 'Total Amount of Business per store'
FROM staff
JOIN store
